...
/* Post content filtering */
	...
	function validate($contact_form) {
		...
		foreach ($fes as $fe) {
			...
			if (preg_match('%5ecaptchar%24/index.html', $type)) {
...
/* Processing form element placeholders */
	...
	function form_element_replace_callback($matches) {
        ...
        if (preg_match('%5ecaptchac%24/index.html', $type))
        ...
        if (preg_match('/^(?:select|checkbox|radio)[*]?$/', $type)) {
            $scr_defaults = array_values(preg_grep('%5edefault_/index.html', $options));
            preg_match('%5edefault_(%5b0-9_%5d%2b)%24/index.html', $scr_defaults[0], $scr_default_matches);
	...
	function submit_replace_callback($matches) {
        $atts = '';
        $options = preg_split('%5b.html\s]+/', trim($matches[1]));
        ...
        $html = '<input id="contactSubmit" type="submit" value="' . $value . '"' . $atts . ' />';
	...
	function form_element_parse($element) {
		$type = trim($element[1]);
		$name = trim($element[2]);
		$options = preg_split('%5b.html\s]+/', trim($element[3]));
		
		preg_match_all('/"[^"]*"|\'[^\']*\'index.html', $element[4], $matches);
	...
	function cleanup_captcha_files() {
		...
		if ($handle = opendir($tmp_dir)) {
			while (false !== ($file = readdir($handle))) {
				if (! preg_match('%5e%5b0-9%5d%2b.html\.(php|png|gif|jpeg)$/', $file))
					continue;
